...
var config = {
  apiOrigins: {
    ...
    development: 'http://tic-tac-toe.wdibos.com' // I ADDED THIS LINE MYSELF. IS IT GOOD?
  }
};
...
var hoverOff = function hoverOff() {
  $('.occupied').hide();
...
var gameOver = function gameOver() {
  var total = 0;
  if (over === false) {
    // this makes sure if someone wins on the last move (which sets over to true) then they won't simultaneously get a game-over message
    for (var key in gameBoard) {
      if (gameBoard[key] !== '') {
        total++;
      }
...
var win = function win() {
  ...
  if (total >= 5) {
    // this is because it takes at least 5 turns total to win the game. So don't even bother checking before then
...
var onCreateGame = function onCreateGame(event) {
  ...
  api.createGame().then(ui.createGameSuccess).then(gameState()).then(reset());
};
...
var updateObject = function updateObject(id) {
...
var onShowGame = function onShowGame(event) {
  ...
  // if (game.id !== '') {
  api.showGame(game.id).then(ui.showGameSuccess)
  // } else {
...
var boardHandlers = function boardHandlers() {
  $('.col-xs-4').hover(hoverOn, hoverOff).on('click', function () {
    updateObject($(this).attr('id'));
  }).on('click', function () {
    move($(this).attr('id'));
  }).on('click', win).on('click', onUpdateGame).on('click', gameOver);
};

var optionsState = function optionsState() {
...
module.exports = {
  gameBoard: gameBoard,
  changeTurns: changeTurns, // probably not necessary
  showX: showX, // probably not necessary
  showO: showO, // probably not necessary
  move: move, // probably not necessary
  onSignUp: onSignUp,
  onSignIn: onSignIn,
  onSignOut: onSignOut,
...
/*
  ...
  client remote, api local
    https://ga-wdi-boston.github.io/first-project/?environment=development
    This will require allowing "unsafe scripts" in Chrome
  client remote, api remote
    https://ga-wdi-boston.github.io/first-project/
*/
